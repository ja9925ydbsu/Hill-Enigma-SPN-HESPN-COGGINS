...
- the 16-round public schedule exercises all 64 labeled seed-orientation pairs equally and was selected for deterministic uniform coverage and reproducibility, not for a demonstrated cryptanalytic advantage;
...
The same 5,000 deterministic plaintext/bit-flip pairs are reused at each reported round count. At 16 rounds the ciphertext Hamming-distance mean is 63.9664 bits with 95% CI [63.81093, 64.12187]. The manuscript uses this only as a bounded whole-harness implementation sanity check: one-bit perturbations are not obviously trapped by the assembled implementation and large-scale output propagation occurs at the tested scale. It is not a differential/linear security bound and not evidence that rotor scheduling is superior to a static layer.
...
The reproducible conclusion is architectural: the order-eight rotor-scheduled Hill-derived linear layer is explicitly specified, locally diffusion-bounded at the stated byte-level granularity, reversible, deterministically schedulable, and usable inside the reported 16-round experimental SPN harness. These materials do not establish a deployment security level, an advantage over AES, an advantage of rotor scheduling over a matched static orientation, or a full-cipher differential/linear bound.
