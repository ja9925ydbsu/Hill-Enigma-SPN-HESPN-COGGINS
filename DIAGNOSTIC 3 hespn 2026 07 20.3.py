...
---------------------
The manuscript's Appendix A test vector uses k=7 for round 0, which
contradicts Section 4.1's stated K_VALUES = [1,3,5,7]. This
implementation follows Section 4.1. The generated test vectors are
internally consistent with the specification in Sections 3-4.
"""
...
K_VALUES = [1, 3, 5, 7]  # As stated in Section 4.1
...
def apply_matrix(byte: int, matrix: List[int]) -> int:
    """Apply 8x8 GF(2) matrix to byte. matrix is list of 8 row-bytes (MSB-first per Section 3.1)."""
    result = 0
    for i in range(8):
...
def main():
    ...
    # (2) Round 0 step-by-step for original plaintext
    ...
    if not all_match:
        print(f"\n    NOTE: If Step 1 differs, check K_VALUES indexing.")
        print(f"          Table A1 says k=7 for r=0, but K_VALUES[0]={K_VALUES[0]}.")
        print(f"          Try K_VALUES = [7, 5, 3, 1] or verify manuscript consistency.")
    ...
    r8 = intermediates[8]
    print(f"Round 8 (r=8, k={K_VALUES[8%4]}, mode={8%4}):")
