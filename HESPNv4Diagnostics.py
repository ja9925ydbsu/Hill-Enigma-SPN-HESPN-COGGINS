...
Parts (toggle with the RUN_* flags below):
  ...
  D. 300-sequence confirmation at 16 rounds
     Reference result: 26 failing tests of 2,700 (expectation 27);
...
# ------------------------------------------------------------------
# Locating the rerun program
# ------------------------------------------------------------------
# Set this to an exact filename (e.g. "Hespnv4rerun20260702.py") to
# force a specific file.  Leave as None for automatic detection:
#   1. HESPNv4Rerun.py, if present in this script's folder;
#   2. otherwise any *.py here whose name contains both "hespn" and
#      "rerun" (case-insensitive) and which defines the required
#      functions (HESPNFastCipher, nist_core_tests,
...
REQUIRED = ("HESPNFastCipher", "nist_core_tests", "hamming_distance_bytes")

def _load_by_path(path):
    spec = importlib.util.spec_from_file_location("HESPNv4Rerun", path)
    mod = importlib.util.module_from_spec(spec)
    sys.modules["HESPNv4Rerun"] = mod
    spec.loader.exec_module(mod)
    return mod

def _find_rerun():
    if RERUN_FILE is not None:
        path = os.path.join(HERE, RERUN_FILE)
        if not os.path.exists(path):
            sys.exit(f"RERUN_FILE = {RERUN_FILE!r} not found in {HERE}")
        return _load_by_path(path), RERUN_FILE
    exact = os.path.join(HERE, "HESPNv4Rerun.py")
    if os.path.exists(exact):
        return _load_by_path(exact), "HESPNv4Rerun.py"
    candidates = sorted(
        f for f in os.listdir(HERE)
        if f.lower().endswith(".py")
        and "hespn" in f.lower() and "rerun" in f.lower()
        and os.path.abspath(os.path.join(HERE, f)) != os.path.abspath(__file__)
    )
    tried = []
    for f in candidates:
        try:
            mod = _load_by_path(os.path.join(HERE, f))
        except Exception as e:
            tried.append(f"{f}: failed to load ({e})")
            continue
        missing = [a for a in REQUIRED if not hasattr(mod, a)]
        if missing:
            tried.append(f"{f}: loaded but missing {missing}")
            continue
        return mod, f
    found = sorted(f for f in os.listdir(HERE) if "hespn" in f.lower())
    sys.exit(
        "Could not locate the v4 rerun program.\n"
        f"  Looked in: {HERE}\n"
        f"  Candidates tried: {tried or 'none matched *hespn*rerun*.py'}\n"
        f"  HESPN-related files found there: {found or 'none'}\n"
        "  Fix: either set RERUN_FILE at the top of this script to the\n"
        "  exact filename of your v4 rerun program, or rename it to\n"
        "  HESPNv4Rerun.py."
    )

m, _rerun_name = _find_rerun()
print(f"[loaded rerun program: {_rerun_name}]")
...
if RUN_D_CONFIRM_300:
    ...
    print("reference: 26/2700 failing vs 27 expected; runs family "
